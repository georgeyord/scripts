...
saveLocallyFromUrl https://github.com/docker/compose/releases/download/${DOCKER_COMPOSE_VERSION}/docker-compose-`uname -s`-`uname -m` /usr/local/bin/docker-compose
chmod +x /usr/local/bin/docker-compose
ensureAppExists "docker-compose"
/usr/local/bin/docker-compose https://raw.githubusercontent.com/docker/compose/$(docker-compose version --short)/contrib/completion/bash/docker-compose /etc/bash_completion.d/docker-compose

echo "Install Docker compose bash completion"
saveLocallyFromUrl https://raw.githubusercontent.com/docker/compose/$(docker-compose version --short)/contrib/completion/bash/docker-compose /etc/bash_completion.d/docker-compose

echo "Install Docker machine"
saveLocallyFromUrl https://github.com/docker/machine/releases/download/${DOCKER_MACHINE_VERSION}/docker-machine-`uname -s`-`uname -m` /usr/local/bin/docker-machine
chmod +x /usr/local/bin/docker-machine
ensureAppExists "docker-machine"

echo "Install Kubernetes's kubectl"
saveLocallyFromUrl https://storage.googleapis.com/kubernetes-release/release/v${KUBECTL_VERSION}/bin/linux/amd64/kubectl /usr/local/bin/kubectl && \
